...
-spec get_items() -> {ok, [item()]} | no_return().
...
-spec add_item(Text :: binary()) -> {ok, ItemId :: item_id()} | no_return().
add_item(Text) ->
...
-spec mark_item(Id :: item_id(), IsDone :: boolean()) -> ok | {error, item_not_found} | no_return().
mark_item(ItemId, IsDone) ->
    todo_list_db:mark_item(ItemId, IsDone).

-spec delete_item(Id :: item_id()) -> ok | {error, item_not_found} | no_return().
